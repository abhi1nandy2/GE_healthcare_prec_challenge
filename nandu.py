...
import keras
from keras.layers import Input, Dense, Conv2D, MaxPooling2D, UpSampling2D, LSTM, ConvLSTM2D, Concatenate
...
total_data = pd.read_csv(str(sys.argv[1]), header = None, usecols = [2], delimiter = '\t')
...
while(i * stride + WL <= anom_data.shape[0]):
	anom_frames.append(np.transpose(scaler.transform(anom_data.iloc[(i * stride):(i * stride) + WL])))
	i=i+1
...
best_model = 'best_model_syn_001.h5'
