...
arr_norm_1 = np.array(pd.read_csv('rrest-syn001_data.csv', usecols = [1], header = None).iloc[0:WL*50]).reshape(-1)
# arr_norm_2 = np.fromfile('normal2_womyo.dat', dtype=float)[0:WL*50]
# myo_1 = np.fromfile('myo1.dat', dtype=float)[0:WL*50]
myo = np.array(pd.read_csv('ark.csv', usecols = ['abnormal']).iloc[0:WL*50]).reshape(-1)
# print(np.shape(arr_norm_1))
data = np.concatenate((arr_norm_1, myo))
...
def load_data(split, BatchSize):
	...
	train_entries, val_entries, train_labels, val_labels = model_selection.train_test_split(img_file_entries, img_labels, test_size = val_split, random_state = 1, stratify = img_labels)
...
# 	encoder.load_state_dict(torch.load('No_crop_equal_clswt_Adam_0.0001_pretr_enc_cat_disc_Seg_Net_' + str(sys.argv[1]) + '_encoder.pt', map_location='cpu'))
# 	print('1 down')
# 	decoder.load_state_dict(torch.load('No_crop_equal_clswt_Adam_0.0001_pretr_enc_cat_disc_Seg_Net_' + str(sys.argv[1]) + '_decoder.pt', map_location='cpu'))
# 	print('2 down')
...
# 	cat_disc.load_state_dict(torch.load('No_crop_equal_clswt_Adam_0.0001_pretr_enc_cat_disc_Seg_Net_' + str(sys.argv[1]) + '_cat_disc.pt', map_location='cpu'))
# 	print('3 down')
...
def train_and_val_net(epoch_nos):
	...
	for epoch in range(iterations):
		...
		for idx, data in enumerate(trainLoader):
			...
			real_imgs = Variable(inputs.cuda())#, requires_grad = True)
		...
		avg_rec_Loss = running_rec_Loss/float(idx + 1)
		avg_cl_Loss = running_cl_Loss/float(idx + 1)
		avg_gen_Loss = running_gen_Loss/float(idx + 1)
		avg_ld_Loss = running_ld_Loss/float(idx + 1)
		avg_cd_Loss = running_cd_Loss/float(idx + 1)
		...
		avg_rec_val_Loss = running_rec_Loss/float(idx + 1)
		avg_cl_val_Loss = running_cl_Loss/float(idx + 1)
		avg_gen_val_Loss = running_gen_Loss/float(idx + 1)
		# avg_ld_val_Loss = running_ld_Loss/float(idx + 1)
		avg_cd_val_Loss = running_cd_Loss/float(idx + 1)
		...
		print('Train Acc: {:.3f} '.format(avg_trainacc*100))
